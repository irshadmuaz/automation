...
var positionElement = function positionElement(elem, x, y, width, height,
	...
	if (animate && $) {
		$(elem).stop();
		$(elem).animate(targetPosition, animate.duration || 200,
				animate.easing || "swing", function() {
					fixAspectRatio();
					if (animate.complete)
...
var arrange = function arrange(children, Width, Height, offsetLeft, offsetTop,
		fixedRatio, minRatio, maxRatio, animate) {
	var count = children.length, availableRatio = Height / Width, vidRatio;
	...
	var getBestDimensions = function getBestDimensions(minRatio, maxRatio) {
		...
		for (var i = 1; i <= count; i++) {
			...
			// If this width and height takes up the most space then we're going
			// with that
	...
	if (!fixedRatio) {
		...
		if (video && video.videoHeight && video.videoWidth) {
			...
			vidRatio = getBestDimensions(3 / 4, 3 / 4); // Use the default video
														// ratio
		}
	}

	var spacesInLastRow = (vidRatio.targetRows * vidRatio.targetCols) - count, lastRowMargin = (spacesInLastRow
			* vidRatio.targetWidth / 2), lastRowIndex = (vidRatio.targetRows - 1)
			* vidRatio.targetCols, firstRowMarginTop = ((Height - (vidRatio.targetRows * vidRatio.targetHeight)) / 2), firstColMarginLeft = ((Width - (vidRatio.targetCols * vidRatio.targetWidth)) / 2);
	...
	for (var i = 0; i < children.length; i++) {
		...
		var actualWidth = vidRatio.targetWidth
		...
		positionElement(elem, x + offsetLeft, y + offsetTop, actualWidth,
				actualHeight, animate);
	}
};
...
var layout = function layout(container, opts, fixedRatio) {
	...
	var Height = getHeight(container) - getCSSNumber(container, "borderTop")
			- getCSSNumber(container, "borderBottom"), Width = getWidth(container)
			- getCSSNumber(container, "borderLeft")
			- getCSSNumber(container, "borderRight"), availableRatio = Height
			/ Width, offsetLeft = 0, offsetTop = 0, bigOffsetTop = 0, bigOffsetLeft = 0, bigRatio = 0, bigOnes = Array.prototype.filter
			.call(container.querySelectorAll("#" + id + ">." + opts.bigClass),
					filterDisplayNone), smallOnes = Array.prototype.filter
			.call(container.querySelectorAll("#" + id + ">*:not(."
	...
	if (bigOnes.length > 0 && smallOnes.length > 0) {
		...
		if (availableRatio > bigRatio) {
			...
			// guys at the bottom
			bigWidth = Width;
			bigHeight = Math.min(Math.floor(Height * opts.bigPercentage), Width
					* bigRatio);
			offsetTop = bigHeight;
			bigOffsetTop = Height - offsetTop;
		} else {
			// We are wide, going to take up the whole height and arrange the
			// small guys on the right
			bigHeight = Height;
			bigWidth = Math.min(Width * opts.bigPercentage, Math
					.floor(bigHeight / bigRatio));
			offsetLeft = bigWidth;
			bigOffsetLeft = Width - offsetLeft;
		}
		if (opts.bigFirst) {
			arrange(bigOnes, bigWidth, bigHeight, 0, 0, opts.bigFixedRatio,
					opts.bigMinRatio, opts.bigMaxRatio, opts.animate);
			arrange(smallOnes, Width - offsetLeft, Height - offsetTop,
					offsetLeft, offsetTop, opts.fixedRatio, opts.minRatio,
					opts.maxRatio, opts.animate);
		} else {
			arrange(smallOnes, Width - offsetLeft, Height - offsetTop, 0, 0,
					opts.fixedRatio, opts.minRatio, opts.maxRatio, opts.animate);
			arrange(bigOnes, bigWidth, bigHeight, bigOffsetLeft, bigOffsetTop,
					opts.bigFixedRatio, opts.bigMinRatio, opts.bigMaxRatio,
					opts.animate);
		}
	} else if (bigOnes.length > 0 && smallOnes.length === 0) {
		// We only have one bigOne just center it
		arrange(bigOnes, Width, Height, 0, 0, opts.bigFixedRatio,
				opts.bigMinRatio, opts.bigMaxRatio, opts.animate);
	} else {
		arrange(smallOnes, Width - offsetLeft, Height - offsetTop, offsetLeft,
				offsetTop, opts.fixedRatio, opts.minRatio, opts.maxRatio,
				opts.animate);
	}
};

otl.initLayoutContainer = function(container, opts) {
	// NOTE: internal OT.$ API
	...
	// this could be left as a responsibility of the user, and i think that
	// would be more sound
